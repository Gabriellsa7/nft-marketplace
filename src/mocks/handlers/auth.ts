...
import { bearerToken, errorResponse, requireSession } from '@/mocks/respond'
import { simulateNetwork } from '@/mocks/scenario'
...
export const authHandlers = [
  http.post('/api/auth/register', async ({ request }) => {
    await simulateNetwork('auth:register')
  ...
  http.post('/api/auth/login', async ({ request }) => {
    await simulateNetwork('auth:login')
  ...
  http.get('/api/auth/session', async ({ request }) => {
    await simulateNetwork('auth:session')
  ...
  http.post('/api/auth/logout', async ({ request }) => {
    await simulateNetwork('auth:logout')
    const token = bearerToken(request)
    const db = getDb()
